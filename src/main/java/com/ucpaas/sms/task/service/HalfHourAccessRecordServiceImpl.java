...
import com.ucpaas.sms.task.strategy.HalfHourAccessRecordEmail;
...
@Service
public class HalfHourAccessRecordServiceImpl implements HalfHourAccessRecordService {
	...
	@Autowired
	private HalfHourAccessRecordEmail halfHourAccessRecordEmail;


	@Override
	public boolean doTheNewJob(TaskInfo taskInfo) throws ParseException {
		return halfHourAccessRecordEmail.doJob(taskInfo);
	}
...
//			SuccessRateValue=Long.valueOf(PropertiesUtil.get("halfEmail_SuccessRateValue"));
//			FakeRateValue=Long.valueOf(PropertiesUtil.get("halfEmail_FakeRateValue"));
//			FailRateValue=Long.valueOf(PropertiesUtil.get("halfEmail_FailRateValue"));
		} catch (Exception e2) {
			PERIOD_TIME = 1800;
//			SuccessRateValue=80;
//			FakeRateValue=5;
//			FailRateValue=8;
		...
		List<ChannelSuccessRateByClientid> oneTableDataList = new ArrayList<ChannelSuccessRateByClientid>(); // 1张流水表的统计数据
		for (int identify = 0; identify < 10; identify++) {
			sqlParams.put("identify", identify);
			sqlParams.put("date", date);
			logger.info("统计表t_sms_record_*, params={}",JacksonUtil.toJSON(sqlParams));
			oneTableDataList = recordSlaveDao.queryAll("smsMonitor.getChannelSuccessRateByClientid", sqlParams);
...
//				}
		...
		List<ChannelSuccessRateByClientid> channelSuccessRateByClientids = channelSuccessRateByClientidMapper.query(channelSuccessRateRealtimeParams);
//		for(ChannelSuccessRateByClientid c:channelSuccessRateByClientids){
//			c.setSuccessRate(c.getSuccessRate().divide(new BigDecimal("100")));
//			c.setFakeSuccessRate(c.getFakeSuccessRate().divide(new BigDecimal("100")));
//			c.setReallyFailRate(c.getReallyFailRate().divide(new BigDecimal("100")));
//		}


		logger.info("获取一天截止到目前的 通道成功率统计数据(record按通道区分)");
...
//		for(ChannelSuccessRateRealtime c:channelSuccessRateRealtimes){
//			c.setSuccessRate(c.getSuccessRate().divide(new BigDecimal("100")));
//			c.setFakeSuccessRate(c.getFakeSuccessRate().divide(new BigDecimal("100")));
//			c.setReallyFailRate(c.getReallyFailRate().divide(new BigDecimal("100")));
//		}
		...
		while(true){
			if(sleepCount >= 5){
				//if(sleepCount >= 1){
				logger.debug("超过5次未找到通道成功率统计的最新数据，结束循环");
				break;
			}

			if(channelSuccessRateRealtimes!=null&&channelSuccessRateRealtimes.size()>0){
				ChannelSuccessRateRealtime csrr = channelSuccessRateRealtimes.get(channelSuccessRateRealtimes.size()-1);
				logger.debug("通道成功率统计的数据，最新一条是{}",JacksonUtil.toJSON(csrr));
				DateTime dataTime = new DateTime(csrr.getDataTime().getTime()); //数据时间是否接近半个小时
				if(dataTime.isAfter(executeNext.minusMinutes(9))&&dataTime.isBefore(executeNext.plusMinutes(9))){
					logger.debug("是最新的通道成功率数据");
					break;
				}
			}
			sleepCount++;
			logger.debug("未找到通道成功率统计的最新数据,休眠5s");
			Thread.sleep(SLEEP_TIME); //不是最新数据，休眠5秒，等通道成功率统计那边的任务跑完
			channelSuccessRateRealtimes = channelSuccessRateRealtimeMapper.query(channelSuccessRateRealtimeParams);
		}
		//只抽半个小时的数据
		...
		while(csrrIt.hasNext()){
			...
			/*long dateLong  = csrr.getDataTime().getTime()/1000;
			if((dateLong%PERIOD_TIME)!=0){
				csrrIt.remove();
			}*/
			...
			//分钟在此区间，说明是整点数据
			if (!((asInt >= 0 && asInt <= 4) || (asInt >= 30 && asInt <= 34))) {
				csrrIt.remove();
			}
		}
		...
		logger.info("获取一天截止到目前的 客户成功率统计数据(access按用户区分)");
		List<ClientSuccessRateRealtime> clientSuccessRateRealtimes =  clientSuccessRateRealtimeMapper.query(channelSuccessRateRealtimeParams);
//		for(ClientSuccessRateRealtime c:clientSuccessRateRealtimes){
//			c.setSuccessRate(c.getSuccessRate().divide(new BigDecimal("100")));
//			c.setFakeSuccessRate(c.getFakeSuccessRate().divide(new BigDecimal("100")));
//			c.setReallyFailRate(c.getReallyFailRate().divide(new BigDecimal("100")));
//		}
		...
		while(true){
			if(sleepCount2 >= 5){
				//	if(sleepCount2 >= 1){
				logger.debug("超过5次未找到客户成功率统计的最新数据，结束循环");
				break;
			}

			if(clientSuccessRateRealtimes!=null&&clientSuccessRateRealtimes.size()>0){
				ClientSuccessRateRealtime csrr = clientSuccessRateRealtimes.get(clientSuccessRateRealtimes.size()-1);
				logger.debug("客户成功率统计的数据，最新一条是{}",JacksonUtil.toJSON(csrr));
				DateTime dataTime = new DateTime(csrr.getDataTime().getTime()); //数据时间是否接近半个小时
				if(dataTime.isAfter(executeNext.minusMinutes(9))&&dataTime.isBefore(executeNext.plusMinutes(9))){
					logger.debug("是最新的客户成功率数据");
					break;
				}
			}
			sleepCount2++;
			logger.debug("未找到客户成功率统计的最新数据，休眠5s");
			Thread.sleep(SLEEP_TIME); //不是最新数据，休眠5秒，等通道成功率统计那边的任务跑完
			clientSuccessRateRealtimes =  clientSuccessRateRealtimeMapper.query(channelSuccessRateRealtimeParams);
		}
		//只抽半个小时的数据
		Iterator<ClientSuccessRateRealtime> csrrIt2 = clientSuccessRateRealtimes.iterator();
		while(csrrIt2.hasNext()){
			ClientSuccessRateRealtime csrr =  csrrIt2.next();
			/*long dateLong  = csrr.getDataTime().getTime()/1000;
			if((dateLong%PERIOD_TIME)!=0){
				csrrIt2.remove();
			}*/
		...
		formatDataAndSend(channelSuccessRateByClientids, channelSuccessRateRealtimes, clientSuccessRateRealtimes);
	...
	private void formatDataAndSend(List<ChannelSuccessRateByClientid> channelSuccessRateByClientids, List<ChannelSuccessRateRealtime> channelSuccessRateRealtimes, List<ClientSuccessRateRealtime> clientSuccessRateRealtimes) {
		//数据获取完后开始组装
...
//        Map<Long,AccessAllHeJi> accessAllHeJiMap = new TreeMap<>();
		...
		for(Entry<Long, AccessAliHeJi> entry:accessAliHeJiMap.entrySet()){
			table1Data.add(entry.getValue());
		}
		for(Entry<Long, AccessAllHeJi> entry:accessAllHeJiMap.entrySet()){
			table1Data.add(entry.getValue());
		}
...
//			all.setReceiveTotal(all.getSendTotal()+all.getSubmitFail());//接收access 1 2 3 4 5 6
		...
		for(Entry<Long, RecordByClientAliHeJi> entry:recordByClientAliHeJiMap.entrySet()){
			table2Data.add(entry.getValue());
		}
		for(Entry<Long, RecordByClientAllHeJi> entry:recordByClientAllHeJiMap.entrySet()){
			table2Data.add(entry.getValue());
		}
		...
		for(Entry<Long, RecordAllHeJi> entry:recordAllHeJiMap.entrySet()){
		...
		for(Entry<String,List<ClientSuccessRateRealtime>> entry:groupByClientId.entrySet()){
		...
		//找最新一条
		for(Entry<String,List<ChannelSuccessRateByClientid>> entry:groupByClientId2.entrySet()){
			//计算半小时
...
//			}
			...
			if(isAli(recordByClientVo.getClientId())){
		...
		//找最新一条
		for(Entry<String,List<ChannelSuccessRateRealtime>> entry:groupByChannelId.entrySet()){
	...
	private String formTable5(List<RecordByClientVo> newlastRecordByClientList) {
		...
		for(RecordByClientVo csrr:newlastRecordByClientList){
			...
			if(csrr instanceof RecordByClientAliHeJi){
				...
				color="color:blue";
			}else if(isAli(csrr.getClientId())){
				color="background-color:#B8B8DC";
			}
			if(dateTime==null)
				dateTime = csrr.getDataTime();
...
//			table.append(csrr.getReallyFailRate().multiply(new BigDecimal("100")).setScale(2, BigDecimal.ROUND_HALF_UP)).append("%");
	...
	public static void main(String[] args) throws IOException {
		HalfHourAccessRecordServiceImpl halfHourAccessRecordService = new HalfHourAccessRecordServiceImpl();
		int emptyTime = halfHourAccessRecordService.getEmptyTime(DateUtil.strToDateLong("2018-01-16 06:30:30"));
